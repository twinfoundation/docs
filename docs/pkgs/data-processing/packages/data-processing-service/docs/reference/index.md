# @twin.org/data-processing-service

## Classes

- [DataProcessingService](classes/DataProcessingService.md)
- [ExtractionRule](classes/ExtractionRule.md)
- [ExtractionRuleGroup](classes/ExtractionRuleGroup.md)

## Interfaces

- [IDataProcessingServiceConstructorOptions](interfaces/IDataProcessingServiceConstructorOptions.md)

## Variables

- [tagsDataProcessing](variables/tagsDataProcessing.md)
- [restEntryPoints](variables/restEntryPoints.md)

## Functions

- [generateRestRoutesDataProcessing](functions/generateRestRoutesDataProcessing.md)
- [dataProcessingSetRuleGroup](functions/dataProcessingSetRuleGroup.md)
- [dataProcessingGetRuleGroup](functions/dataProcessingGetRuleGroup.md)
- [dataProcessingRemoveRuleGroup](functions/dataProcessingRemoveRuleGroup.md)
- [dataProcessingExtract](functions/dataProcessingExtract.md)
- [dataProcessingConvert](functions/dataProcessingConvert.md)
- [initSchema](functions/initSchema.md)
