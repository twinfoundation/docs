...
# How to Create a DID with the TWIN SDK
...
3. **Define Connector Configuration:** Set up the network parameters.
    ...
    const faucetUrl = process.env.IDENTITY_IOTA_FAUCET_ENDPOINT || 'https://faucet.testnet.iota.cafe';
    ```
...
1. **Define your service** parameters.
    ...
    const serviceId = 'linked-domain'; // A unique ID for this service (consistent with repo)
...
1. **Define the credential** subject.
    ...
    const credentialSubject = {
      id: did, // The DID this credential is about
      origin: domainOrigin
    };
    ```
...
2. Fetch and verify the domain configuration JWT.
    ...
    ```javascript
    const origin = (Array.isArray(linked?.serviceEndpoint) ? linked.serviceEndpoint[0] : linked?.serviceEndpoint) || domainOrigin;
    ...
    const cs = Array.isArray(cred.credentialSubject) ? cred.credentialSubject[0] : cred.credentialSubject;
    const subject = cs?.credentialSubject || cs;

    if (issuerDid !== did || subject?.id !== did) throw new Error('DID mismatch in domain linkage credential');
    if (subject?.origin !== normalized) throw new Error('Origin mismatch in domain linkage credential');
